...
use crate::{PE64, pe64::{mapper::{MappedBlock, Mapper}, translation::near::NearTranslation}, psm_error::PSMError};
...
impl Translation {
    ...
    pub fn set_obfuscation_offset(&mut self, obfuscation_offset: u32) {
        match self {
            Translation::Relative(relative_translation) => relative_translation.set_obfuscation_offset(obfuscation_offset),
            Translation::Near(near_translation) => near_translation.set_obfuscation_offset(obfuscation_offset),
            _ => {},
        }
    }

    pub fn buffer_size(&mut self, assume_jumps_are_near: bool) -> Result<usize, iced_x86::IcedError> {
        match self {
            Translation::Near(near_translation) => near_translation.buffer_size(),
            _ => self.buffer(assume_jumps_are_near).and_then(|buffer| Ok(buffer.len())),
        }
    }
    ...
    pub fn translate_rva_to_mapped(pe: &PE64, translations: &[Self], symbols: &[(std::ops::Range<usize>, MappedBlock)], mut rva_to_find: u64) -> Result<u64, PSMError> {
        if pe.is_obfuscated() {
            let target_section = pe.iter_find_section(|section| section.contains_rva(rva_to_find as usize))
                .ok_or(PSMError::RVANotFound(rva_to_find))?;
        
            if target_section.name == ".text" {
                let mut decoder = Decoder::new(64, pe.get_data_from_rva(rva_to_find as usize, 5 /*size of jump*/)?, 0);
        ...
        Translation::find_first_translation_rva(translations, rva_to_find)
            ...
            .ok_or_else(||
                PSMError::TranslationFail(rva_to_find)
            )
